...
import { BarChart3, MapPin, TrendingUp, MessageSquare, AlertTriangle, Bot, Home, Lock, Menu } from 'lucide-react';
...
export const AppSidebar: React.FC<AppSidebarProps> = ({
  ...
  return <TooltipProvider>
      <aside id="sidebar" className={`
        ...
        {/* Header Section */}
        {isOpen && <div className="mb-4 px-2">
        ...
        <nav className="flex flex-col items-center space-y-4">
          ...
          const menuButton = <button onClick={() => !item.disabled && onPageChange(item.id)} disabled={item.disabled} className={`
                  w-11 h-11 rounded-2xl transition-all duration-300 grid place-items-center
                  ${isActive ? 'bg-gradient-to-b from-pink-500 to-rose-500 text-white shadow-md scale-[1.02]' : item.disabled ? 'bg-white/70 opacity-40 pointer-events-none' : 'bg-white/70 hover:bg-white shadow-sm backdrop-blur hover:scale-105'}
                `} aria-label={item.title}>
                <item.icon className={`
                  w-5 h-5 transition-colors duration-300
                  ${isActive ? 'text-white' : item.disabled ? 'text-gray-400' : 'text-[#D81B60]'}
                `} />
                {item.disabled && <Lock className="w-3 h-3 text-gray-400 absolute" />}
              </button>;
          ...
          return <div key={item.id} className="flex items-center gap-3 w-full">
                {menuButton}
          ...
          const aiButton = <button onClick={() => onPageChange(aiAgent.id)} className={`
                  w-12 h-14 rounded-2xl transition-all duration-300 grid place-items-center mt-6
                  ${isActive ? 'bg-gradient-to-b from-pink-500 to-rose-500 text-white shadow-[0_10px_30px_rgba(236,72,153,0.35)] scale-[1.02]' : 'bg-gradient-to-b from-pink-500 to-rose-500 text-white shadow-[0_10px_30px_rgba(236,72,153,0.35)] hover:scale-105'}
                `} aria-label={aiAgent.title}>
                <aiAgent.icon className="w-6 h-6 text-white" />
              </button>;
          if (!isOpen) {
            return <Tooltip key={aiAgent.id}>
                  <TooltipTrigger asChild>
                    {aiButton}
                  </TooltipTrigger>
                  <TooltipContent side="right" className="bg-pink-800 text-white border-pink-600">
                    {aiAgent.title}
                    {aiAgent.badge && <Badge className="ml-2 text-xs bg-pink-600 text-white">
                        {aiAgent.badge}
                      </Badge>}
                  </TooltipContent>
                </Tooltip>;
          }
          return <div key={aiAgent.id} className="flex items-center gap-3 w-full mt-6">
                {aiButton}
