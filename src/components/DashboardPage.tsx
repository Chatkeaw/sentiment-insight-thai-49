...
export default function DashboardPage({ onPageChange }: Props) {
  ...
  // sync เมื่อส่วนอื่นเปลี่ยนเดือน/ปี
  useEffect(() => {
    const handler = (e: any) => {
  ...
  // เขียนกลับไว้ใช้ซ้ำ (ไม่ยิง event ซ้ำ)
  useEffect(() => {
  ...
  return (
    <section className="space-y-5 md:space-y-6">
      {/* แถบหัวข้อ — เว้นระยะสวยงาม + responsive */}
      <div className="px-2 md:px-0">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-2 md:gap-4 mt-1 md:mt-2">
          <h2 className="text-[22px] md:text-3xl font-bold leading-tight tracking-tight text-foreground">
            สรุปภาพรวมประจำเดือน <span className="text-muted-foreground">- {headerLabel}</span>
          </h2>
